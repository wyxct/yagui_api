...
class taskXJ_switching_QLSH():
    def __init__(self):
        self.__name = taskXJ_switching_QLSH.get_name()
        self.cfg = {'cron': '0/2 * * * * * *',
                    'disurl': '{host}/api/p2ptasks/',
                    'desc':"齐鲁石化下架任务下发AGV",'PROJECT_NO':'General'}

    @staticmethod
    def get_name():
        return "taskXJ_switching_QLSH"
    ...
    def save_results(self, data):
        ...
        with sched.SessionFactory() as session:
            result = CronTaskResult(
                job_name=taskXJ_switching_QLSH.get_name(), contents=data)
            session.add(result)
            session.commit()
    ...
    def run(self):
        ...
        import json
        # logger.info(self.__name+ ': taskXJ_switching_QLSH running,'+ time.strftime(
        ...
        for current_task in taskXJ_result:
            try:
                ...
                toloc_check_point_sql = SQL.get_check_point.format(LocationName=current_taskXJ.ToLoc)
                toloc_location_result = run_sql(toloc_check_point_sql)
                if location_result == []:
                    toloc_check_point = None
                else:
                ...
                if code == 200:
                    # 5.插入任务反馈表
                    insert_Task_Sync_sql = SQL.insert_Task_Sync.format(TaskId = current_taskXJ.TaskId, Status = '10', CreateBy = 'taskXJ_s_QLSH')
