            ...
            for i in location_dict['optlist']:
                if i['opt'] == 'load':
                    try:#如果去的位置不存在check点的，直接跳过逻辑
                        ...
                        pass
                    self.aprocess.append({'id': str(start + 1), 'label': 'AGV已经生成前往取货点{}的任务'.format(i['pos']), 'l': str(start)})
                    ...
                    start += 1
                    self.aprocess.append({'id': str(start + 1), 'label': 'AGV正在前往取货点{}'.format(i['pos']), 'l': str(start)})
                    ...
                    try:#如果去的位置不存在check点的，直接跳过逻辑
                        ...
                        pass
                    self.aprocess.append({'id': str(start + 1), 'label': 'AGV已经生成前往卸货点{}的任务'.format(i['pos']), 'l': str(start)})
                    self.mprocess.append({str(start + 1): 'AGV已经生成前往卸货点{}的任务'.format(i['pos']), 'l': str(start)})
                    sql = sql + "when (select 1 from agvcmd where parameter_int4_2 = 1 and agvcmd.agv_command_status_id=10)=1 and (select count(*) from agvcmd where parameter_int4_2 = 1)={} then '{}'\n".format(unload_time, str(start + 1))
                    start += 1
                    self.aprocess.append({'id': str(start + 1), 'label': 'AGV正在前往卸货点{}'.format(i['pos']), 'l': str(start)})
